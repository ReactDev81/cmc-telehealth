...
import { TransactionsData } from "@/json-data/transaction";
    ...
    return(
        <View className='flex-1 bg-white p-5'>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity 
                        onPress={() => router.push(`/profile/transactions/[id]`)}
                        className="flex-row items-center justify-between border-b border-gray-100 py-5"
                    >

                        <View className="flex-row items-center gap-x-3">
                            {item.status === "success" ? (
                                <View className="w-9 h-9 bg-primary-100 items-center justify-center rounded-full">
                                    <Check size={14} color="#013220" />
                                </View>
                            ) : (
                                <View className="w-9 h-9 bg-[#FDE9ED] items-center justify-center rounded-full">
