...
const AddPrescription = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = usePatientDetail(appointment_id || "", token || "");
  ...
  if (isLoading) {
    return (
      <View className="py-20 items-center justify-center">
        ...
        <Text className="text-black-400 mt-4 font-medium">
          Loading medications...
        </Text>
      </View>
    );
      ...
      <View className="pt-5 pb-14 px-5">
        ...
        <View className="mb-7">
          ...
          <View className="flex-row items-center gap-x-2 mt-0.5">
            <Text className="text-sm text-black-400">
              {patientData?.age_display || `${patientData?.age} Years`}
            </Text>
            <View className="w-1 h-1 rounded-full bg-gray mt-0.5"></View>
            <Text className="text-sm text-black-400">
              {patientData?.gender_label || patientData?.gender}
            </Text>
          </View>
        </View>
            ...
            <View className="flex-1 bg-black/40 justify-center items-center">
              <View className="w-11/12 bg-white rounded-2xl overflow-hidden">
                <View className="flex-row items-center justify-between p-5 bg-white border-b border-gray-100">
                  ...
                  <TouchableOpacity
                    onPress={() => setConclusionModalVisible(false)}
                  >
        ...
        <View className="mt-5">
          {currentMedications.length > 0 ? (
            ...
            <Text className="text-black-400 text-sm italic mt-4 text-center">
          ...
          <View className="mt-8 p-4 rounded-xl border border-gray-100">
            ...
            <View className="mb-4">
              <Text className="text-sm text-black leading-5">
                {instructionData.instructions_by_doctor}
              </Text>
            </View>
