...
const AddNewPrescription = ({ onClose }: { onClose: () => void }) => {
    ...
    const { control, handleSubmit, reset, setValue, watch } = useForm<PrescriptionForm>({
    ...
    // Generic dosage options that work for any medicine type
    const dosageOptions = [
        { label: "Half Tablet", value: "0.5 tablet" },
        { label: "1 Tablet", value: "1 tablet" },
        { label: "1.5 Tablets", value: "1.5 tablets" },
    ...
    const submit = (data: PrescriptionForm) => {
        ...
        console.log("📤 Prescription Data Payload:", JSON.stringify(payload, null, 2));

        addPrescription.mutate(payload, {
            onSuccess: (response) => {
                console.log("✅ Prescription Request SUCCESS:", JSON.stringify(response, null, 2));
                setShowSuccess(true);
            },
            onError: (error: any) => {
                console.log("❌ Prescription Request FAILED:", error);
                console.log("❌ Prescription Error Data:", JSON.stringify(error?.response?.data, null, 2));
                ...
                >
                    {watch("medicine_name") ? (
                        <View className="flex-row items-center p-1.5 rounded-xl bg-transparent border border-gray mb-3">
                            <View className="flex-row items-center bg-black-200 px-3 py-1.5 rounded-lg border border-primary-100">
                                <Text className="text-black font-medium mr-2">{watch("medicine_name")}</Text>
                                <Pressable
                                    onPress={() => {
                    ...
                    <View className="mt-4">
                        ...
                        <View className="flex-row flex-wrap gap-x-3 gap-y-2">
                            ...
                            <Checkbox name="timing_evening" label="Evening" control={control} />
                    ...
                    {/* Notes */}
                    <TextArea name="instructions" label="Notes" control={control} containerClassName="mt-4" />

                    <Button onPress={handleSubmit(submit)} className="mt-6 flex-row-reverse mb-4">
