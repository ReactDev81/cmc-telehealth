...
const DoctorDetail = () => {
  ...
  useEffect(() => {
    ...
    if (!opdType) {
      const val =
        schedule?.opd_type ??
        schedule?.consultation_opd_type ??
        schedule?.opdType;
      if (val) {
        const s = String(val).toLowerCase();
  ...
  return (
    <ScrollView className="flex-1 bg-white">
      ...
      <View className="px-5 pt-7 pb-20">
        ...
        <View className="mt-6">
          <Text className="text-lg font-medium text-black">
            ...
            {booking_type === "reschedule" && (
              <Text className="text-xs text-black-400 font-normal">
                {" "}
                (Locked)
              </Text>
            )}
          ...
          <View className="flex-row items-center mt-4 gap-x-4">
            ...
            {doctor?.appointment_types?.in_person && (
              ...
              >
                <Hospital
                  color={appointmentType === "in_person" ? "#fff" : "#4D4D4D"}
                />
                <Text
                  className={`text-sm font-medium text-center mt-2 ${
                    appointmentType === "in_person"
        ...
        {appointmentType &&
          (appointmentType === "video" ||
            (appointmentType === "in_person" && opdType)) && (
            <DoctorSchedule
              doctorData={doctor}
