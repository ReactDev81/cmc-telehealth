...
import { View, Text, TouchableOpacity, LayoutAnimation, Platform, UIManager, AccessibilityProps } from "react-native";
        ...
        <View className="py-3.5 border-b border-[#EDEDED]">
            ...
            >
                ...
                <View className="flex-1">
                    <Text className="text-sm font-medium text-black mt-2">{faq.title}</Text>
                    {expanded && <Text className="text-sm text-[#6B6B6B] leading-6 mt-1">{faq.description}</Text>}
                </View>

                <View className="mt-2">
                    {expanded ? <ChevronDown color="#333" size={20} /> :  <ChevronRight color="#333" size={20} />}
                </View>
            </TouchableOpacity>
        </View>
    );
